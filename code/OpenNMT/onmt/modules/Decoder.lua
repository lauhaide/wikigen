...
function Decoder:__init(inputNetwork, rnn, generator, inputFeed,
    returnAttnScores, tanhQuery, guidedAln)
  self.rnn = rnn
...
function Decoder:copyDTAWeights(dtaModel, dtaTextDict, textDict)
  local dtaEmb, netEmb
  for _, node in ipairs(dtaModel.forwardnodes) do
    if node.data.annotations.name == 'TextEncoder' then
        dtaEmb = node.data.module.modules[1].weight
        --this points to the concat table of the forward and backward nn.SeqLSTM implementation.
        --to copy all internal weigths of the LSTM unit we need to get into each of them (Wxf, Wx,i, etc...) and
        --copy to the corresponding ones tensors in the openNMT implementation.
        --TODO to be done. do a first try only with the embedding initialisation
        --print(node.data.module.modules[2].module.modules[2].modules)
    end
  end

  netEmb = self.inputNet.net.weight
  print("Decoder embeddings weights before: ", netEmb:sum())
  copyFromMaskedLookupTable(netEmb, dtaEmb, textDict, dtaTextDict)
  print("Decoder embeddings weights after: ", netEmb:sum())
...
function Decoder:backward(batch, outputs, criterion, ctxLen)
  ...
  for t = batch.targetLength, 1, -1 do
    ...
    local tgtOutput = batch:getTargetOutput(t) --see here if output is already a table

    local genGradOut
    if self.guidedAln == 1 then
      --{targetOut[{{},t}], alnLabels[{{},t}]}
      local tgtAlnLabels = batch:getTargetAlnLabels(t)
      loss = loss + criterion:forward(pred, { tgtOutput[1], tgtAlnLabels[1]} )
      -- Compute the criterion gradient.
      genGradOut = criterion:backward(pred, { tgtOutput[1], tgtAlnLabels[1]})
    else
      loss = loss + criterion:forward(pred, tgtOutput)
      -- Compute the criterion gradient.
      genGradOut = criterion:backward(pred, tgtOutput)
    end
  ...
  --print(string.format("nnl: %f  ", loss))
...
function Decoder:computeLoss(batch, encoderStates, context, criterion)
  ...
  self:forwardAndApply(batch, encoderStates, context, function (out, t)
    ...
    if self.guidedAln == 1 then
      --{targetOut[{{},t}], alnLabels[{{},t}]}
      local outAlnLabels = batch:getTargetAlnLabels(t)
      loss = loss + criterion:forward(pred, {output[1], outAlnLabels[1]})
    else
...
function Decoder:attnScores(batch, encoderStates, context)
  local encoderStates = encoderStates
    or onmt.utils.Tensor.initTensorTable(self.args.numEffectiveLayers,
                                         onmt.utils.Cuda.convert(torch.Tensor()),
                                         { batch.size, self.args.rnnSize })
