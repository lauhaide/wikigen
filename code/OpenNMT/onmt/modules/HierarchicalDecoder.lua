...
--]]
local HierarchicalDecoder, parent = torch.class('onmt.HierarchicalDecoder', 'onmt.Sequencer')
...
--]]
function HierarchicalDecoder:__init(docRNN, sentenceDecoder, returnAttnScores)
  self.docRNN = docRNN
  --self.inputNet = inputNetwork

  self.sentenceDecoder = sentenceDecoder

  self.args = {}
  self.args.rnnSize = self.docRNN.outputSize
  self.args.numEffectiveLayers = self.docRNN.numEffectiveLayers
...
function HierarchicalDecoder.load(pretrained)
  local self = torch.factory('onmt.HierarchicalDecoder')()
...
function HierarchicalDecoder:serialize()
  return {
...
function HierarchicalDecoder:resetPreallocation()
...
--[[ Build a default one time-step of the text decoder
...
--]]
function HierarchicalDecoder:_buildModel()
  local inputs = {}
  local states = {}
  ...
  --Input will be the representation of previous encoded sentence
  local x = nn.Identity()() --- -- batchSize x rnnSize
  table.insert(inputs, x)
  self.args.inputIndex.x = #inputs

  --TODO: attention at the sentence encoding level???
  --[[
  ...
  local input = x -- input is already
  ...
  local outputs = self.docRNN(states)
...
--]]
function HierarchicalDecoder:maskPadding(sourceSizes, sourceLength)
...
function HierarchicalDecoder:remember()
    self._remember = true
end

function HierarchicalDecoder:forget()
    self._remember = false
end

function HierarchicalDecoder:resetLastStates()
    self.lastStates = nil
end
...
Returns:
 ...
 2. `states` - All states.
--]]
--function HierarchicalDecoder:forwardOne(input, prevStates, context, prevOut, t)
function HierarchicalDecoder:forwardOne(input, prevStates, prevOut, t)
  -- not using context for the moment
  ...
  local outputs = self:net(t):forward(inputs) -- outputs are only states, no attention at this level for the moment -- [states; attnOutput; (attnScores)]
  local attnScoresLoc = self.args.returnAttnScores and 1 or 0
  local out = outputs[#outputs - attnScoresLoc]
  local states = {}
  for i = 1, #outputs do  -- - 1 - attnScoresLoc do NO ATTENTION FOR THIS LEVEL FOR THE MOMENT
...
function HierarchicalDecoder:forwardAndApply(batch, encoderStates, context, func)
  ...
  if self._remember and self.lastStates then
      ...
      --for the moment would like to make it zero, get same strucuture but zeroed
...
--- here handle loop on target document length in number of sentences and target length in
--- number of words for each sentence.

  --initial input for document decoder?
  --TODO: define and verify this! document start symbol????
  local input = onmt.utils.Tensor.initTensorTable(1, self.stateProto, { batch.size, self.args.rnnSize })[1]

  print("states:")
  print(states)
  print("prevOut:")
  print(prevOut)
  for p = 1, batch.targetDocLength do

    print("decoding sentence ", p)
  print(states)
    --for t = 1, batch.targetSentLength do
      --here is forward one for document, at each step of document, do a sentence.
      --prevOut, states = self:forwardOne(input, states, context, prevOut, p)  --leave out context for the moment
      prevOut, states = self:forwardOne(input, states, prevOut, p)
      local pSentBatch = onmt.data.WikiDummyBatch3.new(batch.targetInput[{{},p}], batch.targetOutput[{{},p}], batch:getSource())

    print("decoding words ")
      local decOutputs = self.sentenceDecoder:forward(pSentBatch, encoderStates, context)

      input = decOutputs[#decOutputs]


      -- prepare next step input from decoder's outputs
  ...
  print("first batch target doc done ...")
...
--]]
function HierarchicalDecoder:forward(batch, encoderStates, context)
...
Parameters:
  ...
  * `criterion` - a single target criterion object
  ...
  -- ]]
function HierarchicalDecoder:backward(batch, outputs, criterion, ctxLen)
...
--]]
function HierarchicalDecoder:computeLoss(batch, encoderStates, context, criterion)
...
--]]
function HierarchicalDecoder:computeScore(batch, encoderStates, context)
  local encoderStates = encoderStates
    or onmt.utils.Tensor.initTensorTable(self.args.numEffectiveLayers,
                                         onmt.utils.Cuda.convert(torch.Tensor()),
                                         { batch.size, self.args.rnnSize })
...
function HierarchicalDecoder:greedyFixedFwd(batch, encoderStates, context, probBuf)
