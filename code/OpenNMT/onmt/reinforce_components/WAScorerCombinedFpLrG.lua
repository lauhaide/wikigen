...
local WAScorerCombinedFpLrG = torch.class('WAScorerCombinedFpLrG')

function WAScorerCombinedFpLrG:__init(dict, batch_size)
  self.args = {}
...
function WAScorerCombinedFpLrG:getDynBatch(yLocal, alnsLocal, yGlobal, alnsGlobal, y_pred, reward, reward_mask, blockSampleStart)
  --[[
  -- Computes the reward as F-measure based on Bleu1 unigram precision against the *block* alignment set and
  -- recall against the *document* alignment set.
   ]]
  reward:zero()
  reward_mask:zero()
  ...
  for i = 1, batchSize do
    ...
    if #alnwordsGlobal ~= 0 and last_pos >=2 then
      ...
      local rPrec = get_bleu(tgtPred, alnwordsLocal, 1)
      local rRecall = #alnwordsGlobal ~= 0 and  inter/#alnwordsGlobal or 0
      ...
      if not inspect(r, "r") then
        ...
        print(#tgtPred)
        print(y_pred:size(2))
        print("rew: ", prevr, rPrec, rRecall, r , last_pos)
        os.exit()
      end
